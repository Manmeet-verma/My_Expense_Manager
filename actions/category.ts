...
const createCategorySchema = z.object({
  ...
  subHead: z.string().max(50, "Subhead is too long").optional(),
...
export async function createCategory(data: z.infer<typeof createCategorySchema>) {
  ...
  await prisma.$executeRaw`
    INSERT INTO "Category" ("id", "name", "subHead", "description", "createdAt", "updatedAt")
    VALUES (${randomUUID()}, ${normalizedName}, ${subHead ?? null}, ${description ?? null}, NOW(), NOW())
  `
...
export async function updateCategory(data: z.infer<typeof updateCategorySchema>) {
  ...
  await prisma.$transaction(async (tx) => {
    await tx.$executeRaw`
      UPDATE "Category"
      SET "name" = ${normalizedName}, "subHead" = ${subHead ?? null}, "description" = ${normalizedDescription ?? null}, "updatedAt" = NOW()
      WHERE "id" = ${categoryId}
    `
